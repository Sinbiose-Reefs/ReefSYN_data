...
```{r, echo = F, fig.height=5,fig.width=8,message=F, warning=F}
source("R/packages.R")
source("R/functions.R")
...
# map
wm_data <- wm +  geom_point(data = coords, aes(x = decimalLongitude, y = decimalLatitude), 
...
# map
wm_data_fish <- wm +  geom_point(data = coords_fish, aes(x = decimalLongitude, y = decimalLatitude), 
                            size =4, position= "jitter",
        shape = 19, col = "red4",alpha=0.3) + 

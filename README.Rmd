...
The ReefSYN is a team of 20 early-career and senior researchers, supported by two post-doctoral researchers, from several countries and institutions (Fig. 1). Up to this date, most members of this team performed research in isolation, each one covering a specific dimension of reef biodiversity (Fig. 1). By getting funding and support from Sinbiose we have now the chance to bring together different research expertise/fields/perspectives and be engaged in achieving a common objective: *To provide a synthesis about patterns and drivers of reef diversity and supplying of ecological services in Brazil*. More information about our group can be found at <https://reefsyn.weebly.com/home-us.html>.
...
Over time (actually since 2001), this team of researchers gathered data of fish and benthos in different localities spread throughout the Brazilian coast and oceanic islands. By standardizing these data to a common, globally accepted standard--the Darwin Core Standard--it would be possible to integrate data to answer more ambitious questions about the functioning of Brazilian reef ecosystems, as well quantify, given the distribution of data in space and time, the influence of humans on reef biodiversity and ecosystem services. Below, we make a spatial mapping of our data, and show how the accumulation of sample and taxonomic information in time and space proceeded.

Natural to ecological synthesis is the use of big data (both in terms of amount of data and heterogeneity on data) (WUerst et al. 2019). To achieve our objective, we are analyzing data from almost 400 unique localities distributed throughout the Brazilian coast and oceanic islands. Our data embrace a notable latitudinal range, from 0.91 to -27.6 latitude degrees, and cover with data a significant portion of the Atlantic Ocean. In these localities, we gathered and standardized already published and yet unpublished data of fish and benthic communities to create this large database under the curatorship of the ReefSYN Working Group. In brief, all these data came from geographically replicated, large-scale research programs conducted over the last decades in Brazilian coast and oceanic islands (SISBOTA-MAR, PELD-ILOC, Abrolhos Bank monitoring) or yet novel initiatives such as the monitoring of reef fish and benthos of Rio Grande do Norte. Below we present the map with the spatial distribution of localities aggregated across datasets, for fish and benthic organisms (see legend, Fig. 2). 
...
The dataset presented here reunite information from several independent efforts to characterize the marine biodiversity of the Brazilian province (Fig. 3). In terms of the cumulative efforts, in terms of the cumulative number of eventIDs across the datasets over time, We can see that i) there were an abrupt increase in the number of eventIDs after 2013, mainly for benthos (probably leveraged by the Sisbiota-Mar project (CNPq)), and ii) that the data included in several initiatives (e.g., Sisbiota-Mar, PELD ILOC) gather data from past expeditions (i.e., prior to the grant).
...
table_description_data_benthos <- lapply (list_benthos, function (i)
    ...
    data.frame (samplingProtocol = unique(i$samplingProtocol),
                eventIDs = length(i$samplingProtocol),

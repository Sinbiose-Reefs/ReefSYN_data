...
```{r, echo = F, fig.height=8, fig.width=8, message=F, warning=F, fig.align= "center", out.width="100%", out.height="100%", dpi=300}
...
wm_data <- wm +  geom_jitter(data = coords, 
  ...
  ggtitle (paste ("Number of sites with benthic sampling", "n=", nrow(coords), sep=" "))  +
...
wm_data_fish <- wm + geom_point(data = data.frame(coords_fish), aes(x = decimalLongitude, 
  ...
  theme (plot.background = element_rect("#EBEBEB"),
        ...
        legend.key.width= unit(0.1, 'cm'),
        panel.grid = element_blank())+
  ggtitle (paste ("Number of sites with fish sampling", "n=", nrow(coords_fish), sep=" ")) +
  guides(colour = guide_legend(override.aes = list(alpha=1))) 
...
table_description_data_fish %>% 
  mutate(Sampling.protocol.Var1 = recode(Sampling.protocol.Var1, 
                                         "Underwater visual survey" = "Underwater visual survey - 20 x 2m",
                                         "Underwater Visual Survey - 20 x 2m" = "Underwater visual survey - 20 x 2m",
                                         "video plot" = "Video plots - 1 x 1m",
                                   "Stationary visual survey" = "Stationary visual survey - 4 x 2m",
                                   )) %>% 
  dplyr::rename("Sampling Protocol" = "Sampling.protocol.Var1",
                "Samples (n)" = "Sampling.protocol.Freq",
                "Sampling area (m2)" = "Sampling.area..m2.",
...
```{r, echo = F, fig.height=5,fig.width=8,message=F, dpi=300, warning=F, fig.align= "center",out.width="100%",out.height="100%"}
...
trend_fish_SR <- ggplot (data = acc_time_fish, aes (x=year, y=SR)) + 
  ...
  # projects
  geom_text_repel(data = begin_data_fish, aes (x= min_year, y = 320,
                                                  label = Dataset,
                                                  angle = 0),
                                                  size = 5,
                                                  box.padding=0) + 

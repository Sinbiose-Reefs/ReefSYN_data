...
author: "Reef Synthesis Working Group; Luza, AL, Cordeiro, CAMM, Quimbayo, JP, Aued, AW, Ferreira, CEL, Mendes, TC, Roos, NC, Longo, GO, Francini-Filho, RB, Segal, B, Floeter, SR, Bender, MG"
date: "2023/05/22"
...
```
Fig. 1: Institutions (A) and research topics (B) explored by the ReefSYN team members.
...
ds3<- read.csv (here::here("DwC_output","PELD_iloc_benthos", "event_core.csv"))
...
ds8<- read.csv (here::here("DwC_output","PELD_iloc_fish", "event_core.csv"))
...
The datasets described here represent different efforts to gather data on reef fish and benthos along the Brazilian biogeographical province. We hereby present 13 datasets, being eight of reef fish and five of benthos. Three out of seven fish datasets and two out of five benthic datasets are time series (Tables 1 and 2). The remaining datasets are spatial snapshots (only one visit to a site) whose data were collected over many years.
There were a total of `r total_samples` samples distributed throughout the Brazilian biogeographic province (n=`r samples_fish` for fish, and n=`r samples_benthos`) from `r range_years[1]` to `r range_years[2]`. These samples were distributed in a total of `r sites` sites and `r localities` localities.
...
trend_benthos_annotate <- trend_benthos + geom_text(data = begin_data, aes (x= min_year, y = 2000,
  ...
  ggplot2::annotate("text", x = 2009.5, y = 200, label = "SISBIOTAMar")+
  ggplot2::annotate("text", x = 2015, y = 200, label = "PELD-ILOC")+
  ggplot2::annotate("text", x = 2015, y = 500, label = "ProspecMar")+
  ggplot2::annotate("text", x = 2020.5, y = 850, label = "ReefSYN")
...
trend_fish_annotate <- trend_fish + geom_text_repel(data = begin_data_fish, aes (x= min_year, y = 12000,
  ...
  ggplot2::annotate("text", x = 2009.5, y = 800, label = "SISBIOTAMar")+
  ggplot2::annotate("text", x = 2015, y = 900, label = "PELD-ILOC")+
  ggplot2::annotate("text", x = 2015, y = 2500, label = "ProspecMar")+
  ggplot2::annotate("text", x = 2020.5, y = 3500, label = "ReefSYN")
#+
 # scale_y_reverse()
...
```
Fig. 3: The number of sampling events accumulated over time for fish (top) and benthos (bottom). One sampling event consists, for example, on one transect deployed into a locality. The vertical segments indicate the year in which data included in each dataset started to be collected. The horizontal bars depict the main funding sources as follows: dark gray bar: Sisbiota, CNPq; light gray bar: PELD, CNPq; green bar: ReefSYN, SinBiose CNPq.Databases: I: Fish communities from the Brazilian province; II: Abrolhos Bank monitoring/time series; III: Arraial do Cabo (Rio de Janeiro) monitoring/time series; IV: Oceanic islands’ monitoring/time series; V: Santa Catarina monitoring/time series; VI: Trophic interactions along the Western Atlantic; VII: Alcatrazes monitoring/time series; VIII: Rio Grande do Norte monitoring/time series; IX: Benthic communities’ monitoring in Abrolhos Bank; X: Extended benthic communities from the Brazilian province; XI: Benthic communities from the Brazilian province; XII: Benthic communities’ monitoring/time series from oceanic islands; XIII: Benthic communities from Rio Grande do Norte.
...
ds3_occ<- read.csv (here::here("DwC_output","PELD_iloc_benthos", "DF_occ.csv")) # no meu pc, sep = ";"
...
ds8_occ<- read.csv (here::here("DwC_output","PELD_iloc_fish", "DF_occ.csv"))
...
trend_benthos_SR <- ggplot (data = acc_time, 
  ...
  # grants
  geom_segment(aes(x = 2011, y = -1, xend = 2014, yend = -1),size=3,col="gray30")+
  geom_segment(aes(x = 2013, y = 4, xend = 2023, yend = 4),size=3,col="gray60")+
  geom_segment(aes(x = 2013, y = 12, xend = 2023, yend = 12),size=3,col="#FFA559")+
  geom_segment(aes(x = 2020, y = 20, xend = 2023, yend = 20),size=3,col="#5A8F7B")+
  
  
  ggplot2::annotate("text", x = 2009.5, y = 5, label = "SISBIOTAMar")+
  ggplot2::annotate("text", x = 2015, y = 9, label = "PELD-ILOC")+
  ggplot2::annotate("text", x = 2015, y = 20, label = "ProspecMar")+
  ggplot2::annotate("text", x = 2021.5, y = 30, label = "ReefSYN")
...
trend_fish_SR <- ggplot (data = acc_time_fish, aes (x=year, y=SR)) + 
  ...
  # grants
  geom_segment(aes(x = 2011, y = -1, xend = 2014, yend = -1),size=3,col="gray30")+
  geom_segment(aes(x = 2013, y = 10, xend = 2023, yend = 10),size=3,col="gray60")+
  geom_segment(aes(x = 2013, y = 29, xend = 2023, yend = 25),size=3,col="#FFA559")+
  geom_segment(aes(x = 2020, y = 46, xend = 2023, yend = 46),size=3,col="#5A8F7B")+
  
  
  ggplot2::annotate("text", x = 2009.5, y = 25, label = "SISBIOTAMar")+
  ggplot2::annotate("text", x = 2015, y = 15, label = "PELD-ILOC")+
  ggplot2::annotate("text", x = 2015, y = 40, label = "ProspecMar")+
  ggplot2::annotate("text", x = 2021.5, y = 65, label = "ReefSYN")
...
# N people
N_people<-length(benthos_rec)+length(fish_rec)
...
```
Fig. 4: Trends in the number of scientific names accumulated over time for fish (top) and benthos (bottom). The vertical segments depict the year in which data included in each Dataset started to be collected. The horizontal bars depict the formal funding as follows: dark gray bar: Sisbiota, CNPq; light gray bar: PELD, CNPq; green bar: ReefSYN, SinBiose CNPq. Databases: I: Fish communities from the Brazilian province; II: Abrolhos Bank monitoring/time series; III: Arraial do Cabo (Rio de Janeiro) monitoring/time series; IV: Oceanic islands’ monitoring/time series; V: Santa Catarina monitoring/time series; VI: Trophic interactions along the Western Atlantic; VII: Alcatrazes monitoring/time series; VIII: Rio Grande do Norte monitoring/time series; IX: Benthic communities’ monitoring in Abrolhos Bank; X: Extended benthic communities from the Brazilian province; XI: Benthic communities from the Brazilian province; XII: Benthic communities’ monitoring/time series from oceanic islands; XIII: Benthic communities from Rio Grande do Norte.
...
Data collected by Guilherme Longo and Natália C Roos in Rio Grande do Norte, based on fishes and benthos collected in the same spatial unit. A transect of 20x2m was used to record fishes, they also deployed 10 photo quadrats to register benthic organisms. Data have been collected yearly since 2016, at depths ranging from 1 to 28 m according to local depth. These data were used in a publication by Natalia Roos [Roos et al. 2019](https://www.int-res.com/prepress/m13005.html).
...
The seven fish datasets comprise the description of the occurrence of `r Nspp` taxa from `r Ngenera_fish` genera, `r Nfamfish` families and `r Nclafish` classes, being them `r paste (fish_class[1],fish_class[2], sep="and") `. The five benthic datasets comprise the description of the occurrence of `r Ntaxa`  taxa, `r Ngenera` genera, `r Nfam` families, `r Ncla` classes, and `r Nkin` kingdoms, being them `r king_to_show`. Because the photographic identification does not always allow the species level identification, the number of genera and families are  comparatively high relative to species richness.  In general, the datasets with a large spatial coverage (datasets I and X from SISBIOTA-Mar project) showed higher taxa richness in comparison with local monitoring (datasets IV and XI) (Fig. 6).

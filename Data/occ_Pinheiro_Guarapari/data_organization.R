...
# fish data
guarapari_data_raw <- read.xlsx(here ("Data",
                                 "occ_Pinheiro_Guarapari",
                                 "Guarapari 2008.xlsx"),
                           sheet=1,
                           detectDates =T)
...
guarapari_data <- guarapari_data_raw

# remove the last columns as they are parameters derived from counts and size estimates
guarapari_data <- guarapari_data[,-which(colnames (guarapari_data) %in% c(
                                                        "tamanho.medio.(cm)",
                                                        "biom_censo",
                                                        "abund_m2",
                                                        "biom_m2",
...
abundance_size <- melt (guarapari_data, id.vars = c("censo",
                                  "saida",
                                  "mergulho",
                                  "data",
                                  "natureza", 
...
# into english
dados_bind$ponto2[which(dados_bind$ponto2 == "fora")] <- "offshore"
dados_bind$ponto2[which(dados_bind$ponto2 == "dentro")] <- "inshore"

# paste
dados_bind$ponto2[which(dados_bind$ponto2 %in% c("offshore", "inshore"))] <- paste ("island",
       dados_bind$ponto2[which(dados_bind$ponto2 %in% c("offshore", "inshore"))],
       sep="_")

# paset locality
dados_bind$locality <- paste (dados_bind$ponto2,
                              dados_bind$area,
                                  sep = "_")
...
# creating eventIds
dados_bind$eventID <- paste (paste (paste ("BR:ReefSYN:GUARAPARI-ES:", 
                                           dados_bind$higherGeography,
                                           sep=""),
                                    dados_bind$site,sep=":"),  
                                    dados_bind$locality, 
...
dados_bind$occurrenceID <- paste (paste (paste ("BR:ReefSYN:GUARAPARI-ES:", 
                                                dados_bind$higherGeography,
                                                sep=""),
                                         dados_bind$site,sep=":"),  
                                  dados_bind$locality, 
...
dados_bind$habitat <- dados_bind$natureza
dados_bind$habitat <- ifelse (dados_bind$habitat == "natural",
                              "natural_reef",
                              "artificial_reef")
